...
{
    public partial class frmExcelImportYear : qyFormWithTitle
    {
...
;// * 100 / Total);
        ...
        {
            string where = TDObject.DAOBLL.ImportTBll.WhereSql(tableName, this.cboMonth.Text);
            if (frmLoading)
                where = "Id=-1";
        ...
        private void qyDgv_Table_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                List<string> items = TDObject.DAOBLL.ImportTBll.GetYearMonths(sqlConn,tableName);//TDObject.DAOBLL.ImportTBll.GetYearMonths(tableName);
                ExHlper_CopiedEvent(step_Pbr += 5, stepmax_Pbr);
